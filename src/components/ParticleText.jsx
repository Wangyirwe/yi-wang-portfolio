...
export default function ParticleText({
  ...
  useEffect(() => {
    ...
    const render = (now) => {
      ...
      const idle = !gathering && !pointer.active
      if (idle && now - lastFrame < 34) {
      ...
      const canRepel = pointer.active && !reducedMotion && pointerRepel > 0 && repelRadius > 0
      const canDrift = !gathering && !reducedMotion && idleDrift > 0
      ...
      for (let i = 0; i < n; i += 1) {
        ...
        if (gathering) {
          ...
          progress = local < 0 ? 0 : local > 1 ? 1 : local
    ...
    const sampleText = async () => {
      ...
      const lineGap = resolvedSize * 0.22
      const layouts = lines.map((line) => {
        ...
        return { line, left, right, ascent, descent, width: Math.max(1, left + right), height: Math.max(1, ascent + descent) }
      })
      const padding = Math.max(12, Math.ceil(resolvedSize * 0.08))
      const textWidth = Math.max(1, ...layouts.map((item) => item.width))
      const textHeight = layouts.reduce((sum, item, i) => sum + item.height + (i < layouts.length - 1 ? lineGap : 0), 0)

      offscreen.width = textWidth + padding * 2
      offscreen.height = Math.max(1, Math.ceil(textHeight + padding * 2))
      ...
      layouts.forEach((item) => {
        const x = padding + (textWidth - item.width) / 2 - item.left
        offCtx.fillText(item.line, x, cursorY + item.ascent)
        cursorY += item.height + lineGap
      })
      ...
      const maxParticles = Math.min(1400, Math.max(400, Math.floor((width * height) / 220)))
